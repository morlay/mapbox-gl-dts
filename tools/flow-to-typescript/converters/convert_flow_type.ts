...
export function convertFlowType(node: FlowType): TSType {
  ...
  if (isGenericTypeAnnotation(node)) {
    ...
    const id = node.id;
    if (isIdentifier(id) && id.name === 'Point'){
      return tsTypeReference(identifier('typeof import("@mapbox/point-geometry").default'))
    } else if (isIdentifier(id) && id.name === 'VectorTile'){
      return tsTypeReference(identifier('typeof import("@mapbox/vector-tile").VectorTile'))
    } else if (isIdentifier(id) && id.name === 'VectorTileLayer'){
      return tsTypeReference(identifier('typeof import("@mapbox/vector-tile").VectorTileLayer'))
    } else if (isIdentifier(id) && id.name === 'VectorTileFeature'){
      return tsTypeReference(identifier('typeof import("@mapbox/vector-tile").VectorTileFeature'))
